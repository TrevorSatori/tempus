...
<body>
    <div class="snap-center">
        <center>
            ...
            {#if !isFocused}
                <button class="btn btn-success" on:click={startFocus}>Focus</button>
            {:else}
                <button type="submit" class="btn btn-error" on:click={stopFocus}>Stop</button>
            {/if}
        </center>
    </div>
    ...
    {#if totalTime < 60 && isFocused}
        <h3>Sessions less than a minute won't be recorded</h3>
    {/if}
</body>